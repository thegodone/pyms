...
def ms_lib_match(ms_lib, ms):
    ...
    """
    @summary: Matches a mass spectrum the records in the mass spectral
        library

    @para ms_lib: Mass spectral library
    @type ms_lib: pyms.libMS.JCAMP.Class.MSLib
    @para ms: The query mass spectrum
    ...
    for record in ms_lib.records:
        ...
        if len(record.mass_record) == 0:
            error("library mass record with no mass spectrum")
        ...
        for item in record.mass_record:
            if item[0] >= ms.mz[0] and item[0] <= ms.mz[len(ms.mz) - 1]:
